...
def test_antigen_process_fetch_rejects_scalar_json_shape():
    ...
    mock_resp.status_code = 200
    mock_resp.headers = {}
    mock_resp.iter_content = lambda chunk_size=65536: iter([b"42"])
    mock_resp.close = MagicMock()
...
def test_antigen_process_fetch_closes_streamed_error_response():
    mock_resp = MagicMock()
...
def test_antigen_fetch_payload_closes_l402_challenge(monkeypatch):
    ...
    mock_resp.headers = {"WWW-Authenticate": 'L402 macaroon="m", invoice="i"'}
    mock_resp.close = MagicMock()

    with patch.object(nostr, "_fetch_response", return_value=mock_resp):
        with pytest.raises(FetchError) as exc:
